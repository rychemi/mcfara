...
let goal;
let proximityCheck = true; //check if new goal is in 1 cell proximity of the player
let score = 0;
...
class Maze {
  ...
  generate() {
    ...
    goal.col = Math.floor(Math.random() * this.cols);
    goal.row = Math.floor(Math.random() * this.rows);
    if (proximityCheck) {
      while (Math.abs(goal.col - player.col) <= 1 && Math.abs(goal.row - player.row) <= 1) {
        goal.col = Math.floor(Math.random() * this.cols);
        goal.row = Math.floor(Math.random() * this.rows);
      }
    }
  ...
  //check if player reached goal and if so, generate new maze
  checkEnd() {
    if (player.col == goal.col && player.row == goal.row) {
      //alert("You Reached Finish!");
      score++;
  ...
  redraw() {
    ...
    ctx.fillRect((goal.col) * this.cellSize, (goal.row) * this.cellSize, this.cellSize, this.cellSize);
...
function onClick(event) {
  maze.cols = maze.rows = document.getElementById("size").value;
